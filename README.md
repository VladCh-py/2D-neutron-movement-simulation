...
2d Monte-carlo Neutrons movement simulation.py allows you to simulate the motion of neutrons in a 2D space with three environments:
...
Neutrons are born with a random position and a random direction inside a given environment.
...
the motion of neutrons in environments and a histogram showing the flow 
(calculated as the number of neutrons per unit area) of netrons along the abscissa axis.


*In this version, it is possible to use the reflector in a different way: If you uncomment the marked code pieces, the neutron may reflect off the reflector.
